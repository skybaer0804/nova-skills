...
// ── Instruction queue (Telegram → agent) ──────────────────────────────────────
// Messages from Telegram are pushed here; agent calls /receive to drain the queue
...
bot.command('status', async ctx => {
  const q = instructionQueue.length;
  await ctx.reply(`🟢 브릿지 실행 중 | Port: ${PORT} | Chat ID: ${chatId ?? '미등록'} | 대기 지시: ${q}개`);
});

// Any plain text message → push to instruction queue
bot.on('message:text', async ctx => {
  ...
  await ctx.reply(`📥 지시 접수됨 (큐 ${instructionQueue.length}개)\n에이전트가 곧 처리합니다.`);
  console.log(`[telegram] instruction queued: "${instruction.text}"`);
});
  ...
  req.on('end', async () => {
    try {
      ...
      if (req.url === '/ping') {
        ...
        res.end(JSON.stringify({ ok: true, chatId, queued: instructionQueue.length }));
        return;
      }

      // Drain instruction queue — returns all pending instructions and clears the queue
      if (req.url === '/receive') {
        const instructions = instructionQueue.splice(0);
        res.writeHead(200, { 'Content-Type': 'application/json' });
